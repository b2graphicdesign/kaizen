class Patient::StepsController < ApplicationController
  ...
  def patient_params(step)
    permitted_attributes = 
      case step
      when "basic"
        [:first_name, :last_name, :address_1, :address_2, :city, :state, :county] 
      when "payment"
        [:payer, :payer_id, :payer_state, :transportation_type]
      when "contact"
        [:phone, :email, :alert_call, :alert_sms, :alert_email, :alternate_contact_name, :alternate_contact_phone, :alternate_contact_email, :alert_alternate_call, :alert_alternate_sms, :alert_alternate_email]
      end
